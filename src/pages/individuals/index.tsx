...
type ColumnType = ColumnDef<IndividualInterface, unknown>;
...
export function IndividualListPage(props: IndividualListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<IndividualInterface>>(
    () => `/individuals?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: IndividualInterface) => {
    if (hasAccess('individual', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/individuals/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/individuals/view/${record.id}`} passHref legacyBehavior>
            <Button
